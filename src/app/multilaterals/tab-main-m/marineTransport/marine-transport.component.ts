...
export class MarineTransportComponent implements OnInit {
  ...
  private getIndicatorMultilateral(code: string) {
    this.indicadorMultilateralService.getIndicatorMultilateralByCode(code).subscribe(
      (response) => {
        ...
        this.years = Utilidades.getYears(this.dataMarine.series, 0)
        this.countries = Utilidades.getCountries(this.dataMarine.series, 1);
  ...
  getDataLeaked(data: any[]) {
    ...
    let aData: any[] = [];
    let serie = this.dataMarine.series;
